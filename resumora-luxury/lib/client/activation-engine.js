...
function emailsMatch(actor, stripeEmail) {
  const profile = normalizeEmail(actor?.profileEmail);
  const checkout = normalizeEmail(stripeEmail);
  if (!checkout || !profile) return true;
  return profile === checkout;
}

function signedInStripeFailure(actor, sessionId, failedStep, logs) {
  const signedIn = Boolean(actor?.profileId);
  if (signedIn) {
    logs.push(logEntry("recovery_required", false, { failedStep, signedIn: true }));
    return {
      ok: false,
      ...
      signedIn: true,
      hasAccess: false,
      redirectTo: "/studio?recovery=activation",
      logs,
    };
  }
  return {
...
function buildEmailMismatchPayload(actor, stripeEmail, sessionId, logs) {
  logs.push(
    logEntry("auth_email_mismatch", false, {
      profileEmail: normalizeEmail(actor?.profileEmail),
      stripeCheckoutEmail: normalizeEmail(stripeEmail),
    })
  );
  console.error("[activation-engine] auth_email_mismatch", {
  ...
  return {
    ok: false,
    activationStatus: "email_mismatch",
    activationSuccess: false,
    phase: "failed",
    ...
    stripeCheckoutEmail: normalizeEmail(stripeEmail),
    email: normalizeEmail(actor?.profileEmail),
    redirectTo: `/login?next=${encodeURIComponent(`/studio?session_id=${encodeURIComponent(sessionId)}`)}`,
    logs,
  };
}
...
async function tryCompleteFromDatabase(actor, sessionId, lang, sql, logs) {
  ...
  if (!emailsMatch(actor, stripeEmail)) {
    await setRunPhase(sql, sessionId, "failed", { failureReason: "auth_email_mismatch", customerEmail: stripeEmail });
    return buildEmailMismatchPayload(actor, stripeEmail, sessionId, logs);
  }
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    if (!session && stripe) {
      try {
        ...
        return signedInStripeFailure(actor, sid, "stripe_lookup_failed", logs);
      }
      ...
      return signedInStripeFailure(actor, sid, "stripe_unconfigured", logs);
    }
    ...
    if (session && session.payment_status !== "paid") {
      ...
      return {
        ...
        redirectTo: "/studio?recovery=session",
        logs,
      };
    }
    ...
    if (!activation.planId || !activation.planActivated) {
      ...
      return signedInStripeFailure(actor, sid, "entitlement_grant", logs);
    }
    ...
    if (actor?.profileId) {
      const checkoutEmail = normalizeEmail(
        activation.stripeCheckoutEmail ||
          session?.customer_details?.email ||
          session?.customer_email
      );
      if (!emailsMatch(actor, checkoutEmail)) {
        await setRunPhase(sql, sid, "failed", {
          failureReason: "auth_email_mismatch",
          customerEmail: checkoutEmail,
        });
        await appendRunLog(sql, sid, logEntry("auth_email_mismatch", false, { checkoutEmail }));
        return buildEmailMismatchPayload(actor, checkoutEmail, sid, logs);
      }
