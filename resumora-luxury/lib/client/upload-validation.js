...
/** 0 or unset = unlimited (luxury client uploads). Positive env caps size in bytes. */
function getMaxUploadBytes() {
  const raw = process.env.CLIENT_UPLOAD_MAX_BYTES;
...
function validateUploadFile({ originalFilename = "", mimetype = "", size = 0, filePath = "" } = {}) {
  ...
  if (maxBytes != null && Number(size) > maxBytes) {
    return { ok: false, code: "file_too_large", error: "file_too_large", maxBytes };
  }
