...
function Write-PRAEAuthorizedExecution {
    ...
    $line = "[$([DateTimeOffset]::UtcNow.ToString("o"))] AUTHORIZED | SCOPE:$ExecutionScope | TOKEN:$($Token.Token.Substring(0,20))... | FP:$($Fingerprint.Fingerprint.Substring(0,16))..."
...
function Invoke-PRAEFullRuntimeValidation {
    ...
    # 9. Ledger writable
    try {
        $testLine = "[$([DateTimeOffset]::UtcNow.ToString("o"))] VALIDATION_TEST | PRAE full runtime validation ledger write test"
        $logDir = Split-Path $Script:VIOLATIONS_LOG -Parent
